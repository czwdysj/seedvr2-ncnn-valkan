...
长视频推理还缺少哪些工作。文中性能数据来自真实完整程序，不把单算子数据冒充端到端
结果。
...
这个项目覆盖的不是单一 CUDA/Vulkan kernel，而是一条完整推理系统链路：模型阅读、
计算图边界设计、pnnx/NCNN 模型转换、自定义算子、GPU 内存与命令调度、跨后端数值
验证、性能剖析、C++ API 设计和可复现发布。

面试中最有价值的讨论点不是“成功转换了 3B 模型”，而是能够用证据回答：为什么这样
拆图、哪些语义不能静态化、误差如何逐层定位、为什么 GPU layer 仍可能发生 CPU 往返、
如何区分计算耗时和冷启动耗时，以及下一项优化为什么应由 profile 而不是直觉决定。

## 12. 总结

SeedVR2 NCNN Vulkan v2.0 已经是一个可在受支持环境中从源码构建、下载模型并完成
真实媒体推理的闭环项目。它当前最突出的成果是复杂动态视频模型的可验证转换、六类
CPU/Vulkan 自定义层以及 Engine 级 VkMat 常驻数据流。

它还不是成熟的通用视频产品：长视频、半精度高性能路径、冷启动和跨 GPU 兼容性仍需
继续建设。当前工程的价值在于已经建立了正确、可测、可继续优化的底座，并且每一项
性能结论都有明确测试口径。
