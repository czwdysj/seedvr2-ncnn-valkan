...
namespace seedvr2
{
    namespace
    {
        ...
        float transformed_condition_timestep(float noise_scale, int frames, int height, int width)
        {
            ...
            const float x1 = 256.0f * 256.0f * 37.0f;
            const float x2 = 1280.0f * 720.0f * 145.0f;
            const float shift = 1.0f + (static_cast<float>(height * width * frames) - x1) * 4.0f / (x2 - x1);
            const float normalized = timestep / 1000.0f;
            return 1000.0f * shift * normalized / (1.0f + (shift - 1.0f) * normalized);
        }
    ...
    public:
                    ...
                    Video &output)
        {
            ...
            if (options.cfg_scale != 1.0f && (!negative_used.valid() || negative_used.channels != 5120))
                return fail(Status::InvalidArgument,
            ...
            if (condition.empty() || positive_text.empty() || (options.cfg_scale != 1.0f && negative_text.empty()))
                           ...
                           Video &output)
        {
            ...
            if (posterior_flat.empty() || latent_flat.empty() || augment_flat.empty() || positive_text.empty() || (options.cfg_scale != 1.0f && negative_text.empty()))
            ...
            const float condition_ratio =
                transformed_condition_timestep(options.condition_noise_scale, frames, height, width) / 1000.0f;
            ...
            if (result != 0 || condition_command.submit_and_wait() != 0)
                return result != 0 ? result : fail(Status::InferenceFailed, "Vulkan condition dispatch failed");
            ...
            for (std::size_t index = 0; index < timesteps.size(); ++index)
            {
                ...
                if (result != 0 || prepare_command.submit_and_wait() != 0)
                    return result != 0 ? result : fail(Status::InferenceFailed, "Vulkan DiT input dispatch failed");
                ...
                if (result != 0 || sample_command.submit_and_wait() != 0)
                    return result != 0 ? result : fail(Status::InferenceFailed, "Vulkan sampler dispatch failed");
                latent = next;
            }
    ...
    bool Video::valid() const noexcept
    {
        ...
        return checked_element_count(frames, height, width, channels, expected) && data.size() == expected && std::isfinite(fps) && fps > 0.0f;
    }
    ...
    const char *status_message(Status status) noexcept
    {
        ...
        case Status::NotLoaded:
            return "engine is not loaded";
        case Status::ModelNotFound:
            return "model file not found";
        case Status::ModelLoadFailed:
            return "model load failed";
        case Status::InferenceFailed:
